...
TODO: add more infomration about how to use the context field in our text
...
However, even though both terms are similar, they are behaving differently.
Maybe `achievement scope` would have been a better name, but it's hard to change as of now.
...
1. **Global** \
   The user-achievement relation will always be null, as it should take all events of related metrics into account.
2. **Global_Matches** \
   Relation will just be `match` and means that it will take all events related to all matches, but no courses and nothing else into account.
3. **Global_Courses** \
