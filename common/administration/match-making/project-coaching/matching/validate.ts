...
function ensureExistence(coach: Student, coachee: Pupil) {
    return !!coach && !!coachee;
}
function ensureStillActive(coach: Student, coachee: Pupil) {
    return coach.active && coachee.active;
}
function ensureRoles(coach: Student, coachee: Pupil) {
    return coach.isProjectCoach && coachee.isProjectCoachee;
}
function ensureOpenMatchRequests(coach: Student, coachee: Pupil) {
...
async function ensureNeverPreviouslyMatchedTogether(coach: Student, coachee: Pupil, manager: EntityManager) {
    ...
    const reloadProjectMatches = pm => reloadProjectMatchesInstances(pm, manager);
