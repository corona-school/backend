...
const certRequest = test("Student requests Certificate", async () => {
    ...
    expectFetch({
        ...
        "method": "POST",
        "body": `{\"SandboxMode\":true,\"Messages\":[{\"From\":{},\"To\":[{\"Email\":\"${pupil.email.toLowerCase()}\"}],\"TemplateID\":2317254,\"TemplateLanguage\":true,\"Variables\":{\"certificateLink\":\"lernfair-user-app-dev.herokuapp.com\",\"pupilFirstname\":\"${pupil.firstname}\",\"studentFirstname\":\"${student.firstname}\"},\"TemplateErrorReporting\":{\"Email\":\"backend@lern-fair.de\"}}]}`,
        "responseStatus": 200,
        "response": "{}"
    });
    ...
    const { me: { student: { participationCertificates } }} = await studentClient.request(`
...
const signedCert = test("Pupil signs Certificate", async () => {
    ...
    const { me: { pupil: { participationCertificatesToSign }}} = await pupilClient.request(`
