...
test("Token Request", async () => {
    const { client, pupil: { email, pupil: { id } } } = await pupilOne;
    ...
    const { me: { pupil: { id: id1 }} } = await otherDeviceClient.request(`
        query CheckLoggedIn {
            me { pupil { id } }
        }
    `);
    ...
    const { me: { pupil: { id: id2 }} } = await otherDeviceClient.request(`
        query CheckLoggedIn {
            me { pupil { id } }
        }
    `);
