...
export class UserFieldsResolver {
    ...
    @Authorized(Role.OWNER, Role.ADMIN)
    async concrete_notifications(@Root() user: User): Promise<ConcreteNotification[]> {
        return [
            { id: 1, context: {}, notificationID: 1, sentAt: new Date(), userId: user.userID, state: ConcreteNotificationState.SENT },
            // TODO: More mocking
        ];
    }
