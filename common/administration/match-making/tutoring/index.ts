...
export async function matchMakingWithPersons(
    tutorsToMatch: Student[],
    tuteesToMatch: Pupil[],
    ...
    if (!options.dryRun) {
        ...
        const savedDatabaseMatches = await manager.transaction(async (transactionManager) => {
            //do this in a transaction, such that all or no matches were saved
        ...
        return {
            //if dry run, only query the pupil and student pairs from the database and return them
            ...result,
...
export async function matchMakingOfAllPossibleMatches(
    manager: EntityManager,
    restrictToThoseWithConfirmedInterest: boolean,
    options: MatchMakingOptions = { dryRun: false, silent: false }
) {
