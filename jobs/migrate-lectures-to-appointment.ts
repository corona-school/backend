...
export default async function execute() {
    const lectures = await prisma.lecture.findMany({where: {appointmentType: "legacy"}, select: {id: true, subcourseId: true, matchId: true, subcourse: true }});

    const updatedLectures = await Promise.all(lectures.map(async (lecture) => {
        if (Boolean(lecture.subcourseId)) {
            const participants = (await prisma.subcourse_participants_pupil.findMany({where: {subcourseId: lecture.subcourseId}, select: {pupilId: true}})).map((participant) => `pupil/${participant.pupilId}`);
            const instructors = (await prisma.subcourse_instructors_student.findMany({where: {subcourseId: lecture.subcourseId}, select: {studentId: true, student: true}}));
