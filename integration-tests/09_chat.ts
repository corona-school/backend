...
void test('Create Chat for Course', async () => {
    ...
    expectFetch({
        ...
        method: 'PUT',
        body: `{"subject":"Wie schreibe ich Integrationstests","custom":{"start":"${courseDate}","groupType":"${ChatType.NORMAL}","subcourse":"[${subcourseId}]"},"participants":["pupil_${pupil.pupil.id}","pupil_${pupil2.pupil.id}","student_${instructor.student.id}"]}`,
        responseStatus: 200,
    });
